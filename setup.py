...
setup(
    name="qudi-alazar",  # Choose a custom name
    version=version,  # Automatically deduced from "VERSION" file (see above)
    packages=find_namespace_packages(
        where="src"
    ),  # This should be enough for 95% of the use-cases
    package_dir={"": "src"},  # same
    package_data={"": []},  # include data files
    description="Qudi modules for Alazar card data collection.",  # Meaningful short(!) description
    ...
    keywords=[
        "qudi",  # Add tags here to be easier found by searches, e.g. on PyPI
        "experiment",
        "measurement",
        "framework",
        "lab",
    ...
    install_requires=windows_dep
    if sys.platform == "win32"
    else unix_dep,  # package dependencies
