...
__all__ = ["AlazarPhotothermalAVIControlGui"]
...
from qudi.logic.photothermal_avi_logic import PhotothermalAVISettings, PhotothermalAVILogic
import os
...
class PiezoControlWindow(QtWidgets.QMainWindow):
    """Main window for Photothermal AVI measurement"""

    def __init__(self, *args, **kwargs):  # type: ignore
        super().__init__(*args, **kwargs)  # type: ignore
        self.setWindowTitle("Alazar Photothermal AVI Control")
...
class AlazarPhotothermalAVIControlGui(GuiBase):
    """Qudi GUI module Alazar Photothermal AVI control"""
    _logic = Connector(name="photothermal_avi_logic", interface="PhotothermalAVILogic")

    # window handle (helps the type checker)
    _mw: PiezoControlWindow

    # Signals
    sigStartMeasurement = QtCore.Signal()
    sigStartLiveMeasurement = QtCore.Signal()
    sigStopMeasurement = QtCore.Signal()
    sigUpdateAcquisitionSettings = QtCore.Signal(object)  # PhotothermalAVISettings
    sigSaveData = QtCore.Signal()

    def on_activate(self):
        self._running = False
        self._mw = PiezoControlWindow()
        logic: PhotothermalAVILogic = self._logic()
        ...
        self._mw.live_process_fn.setText(
            settings.live_process_function if settings.live_process_function is not None else ""
        )
        self._mw.end_process_fn.setText(
            settings.end_process_function if settings.end_process_function is not None else ""
        )
        self._mw.save_folder.setText(
            settings.autosave_file_path if settings.autosave_file_path is not None else ""
        )
    ...
    def _wavelengths_per_pixel(self, num: int):
        settings: PhotothermalAVISettings = self._logic().experiment_info
